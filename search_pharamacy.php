        ...
        <div id="filt_table" class="CSSTableGenerator container"> 
            <table id="testTable"  class="table table-bordered">
                <?php
                    ...
                    $sql2 = "SELECT * from pharamacy where cancel = '0' order by pharmcyno  desc";
   
                echo "<table id='example'  class='table table-bordered' style='font-size: 14px;'>";

                echo "<thead><tr>";
                echo "<th>Pharamacy No</th>";
                echo "<th>Pation Name</th>";
                echo "<th>Grand Tot</th>"; 
                echo "<th>Date</th>"; 
                ...
                foreach ($conn->query($sql2) as $row) { 
                    $cuscode = $row['pharmcyno']; 
                         echo "<tr>               
                              <td onclick=\"custno('$cuscode', '$stname');\">" . $row['pharmcyno'] . "</a></td>
                              <td onclick=\"custno('$cuscode', '$stname');\">" . $row['cusname'] . "</a></td>
                              <td onclick=\"custno('$cuscode', '$stname');\">" . $row['grandtot1'] . "</a></td>
